...
$videoFilePath = $_FILES["video"]["tmp_name"];

// Read the uploaded video file
$videoData = file_get_contents($videoFilePath);

// Validate the HMAC signature of the video
$secretKey = 'KOSMOS_TEST'; // Shared secret key (must match the one used to generate the HMAC)

// Use exiftool to extract the "Kosmos" metadata field
$command = "exiftool -Kosmos -b " . escapeshellarg($videoFilePath);
$originalHMAC = trim(shell_exec($command));

if (!$originalHMAC) {
    echo json_encode(["success" => false, "message" => "Video is not recorded with a Kosmos system or it was altered."]);
    exit;
}

// Calculate the HMAC of the uploaded video content
$calculatedHMAC = hash('sha256',$secretKey);

// Compare the calculated and original HMAC signatures
if (!hash_equals($calculatedHMAC, $originalHMAC)) {
    echo json_encode(["success" => false, "message" => "The HMAC signature is invalid. Video upload rejected."]);
    exit;
}

// Calculate a SHA-256 hash of the video file to ensure uniqueness
$videoHash = hash("sha256", $videoData);
